...
#if DEBUG
		[HarmonyPostfix, HarmonyPatch(typeof(OxygenManager), nameof(OxygenManager.RegisterSource))]
		static void registerOxygenSource_Postfix(Oxygen src) => $"registering oxygen source {src.name}".onScreen().logDbg();

		[HarmonyPostfix, HarmonyPatch(typeof(OxygenManager), nameof(OxygenManager.UnregisterSource))]
